...
def data_enquiry(DB_HOST,DB_PORT,DB_NAME,DB_USER,DB_PASSWORD):
    ...
    for table in tables['table_name']:
        ...
        # Read Data from table and file in the dictionary
    ...
    # heading
    for i in range(len(database_enquiried.keys())):
        table_name = list(database_enquiried.keys())[i]
...
def outlier_check(tables_dict):
    ...
    if "trades" in tables_dict and "users" in tables_dict:
        ...
        unmatched_rows_trades = pd.DataFrame()
        unmatched_rows_users = pd.DataFrame()
    
        # Perform a left join to find unmatched data with `login_hash + server_hash`
        ...
        unmatched_users = user_df.merge(trades_df, on=["login_hash", "server_hash"], how="left", indicator=True)
        unmatched_users = unmatched_users[unmatched_users["_merge"] == "left_only"].drop(columns=["_merge"])
        ...
        if not unmatched_trades.empty:
            ...
            unmatched_rows_trades = pd.concat([unmatched_rows_trades, unmatched_trades])
        
        if not unmatched_users.empty:
            print(f"Found {len(unmatched_users)} records where `login_hash + server_hash` in `users` do not exist in `trades`")
            unmatched_rows_users = pd.concat([unmatched_rows_users, unmatched_users])
    ...
    for table_name, df in tables_dict.items():
        ...
        # **(7) Check if holding time exceeds 1 year**
        if "open_time" in df.columns and "close_time" in df.columns:
    ...
    return unmatched_rows_trades,unmatched_rows_users,issues_dict,problematic_rows_dict,warning_rows_dict
...
if __name__ == "__main__":
    ...
    data_got = data_enquiry(host,port,name,user,password)
    [unmatched_trades,unmatched_user,issues,problematic,warning] = outlier_check(data_got)
...
'''
************ Comments ************
This assessment focuses on validating the integrity, consistency, 
and correctness of the data across multiple dimensions.
...
1.Join Integrity Issues
  ...
  824 records in the users table have login_hash + server_hash that do not exist in the trades table.
  Potential Issue: This discrepancy may indicate data integrity issues, such as missing user registrations,
                    orphaned trade records, or synchronization failures between the trades and users tables.

2. Anomalies in the trades Table
  String Format Issues:
     Column symbol contains 2 rows with unexpected special characters "USD,CHF".
     Impact: These may indicate incorrect symbol encoding or data entry errors.
  Volume-Contract Size Mismatch:
     1 row where volume = 0 but contractsize = 100,000.
...
4. Regulatory Considerations
  Australian Financial Regulation Compliance:
     ...
     Impact: While no regulatory threshold was identified, monitoring large 
             ...
             and compliance with AML requirements.
5. Holding Time Issues:
...
incorrect values, missing data, extremely high values, and duplicates. The primary concerns are:
  ...
  2. Potential incorrect transaction data, including volume inconsistencies.
  3. Data entry errors in symbols and missing contract sizes.
  4. User dataset duplication, which may affect relational data accuracy.
  5. No specific reporting threshold found in Australian regulations, but monitoring large transactions remains a key practice.
  6. long holding periods is not an anomalies, but may need further investigation.

Recommended Actions:

Investigate why login_hash + server_hash mismatches exist and whether the missing user records need to be restored.
Verify and fix special character anomalies in the symbol column.
...
Remove duplicate user records to prevent inconsistencies.
